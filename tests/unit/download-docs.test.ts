...
describe("processFile", () => {
  it("Should process and save a .mmd file as .md", async () => {
    ...
    const mockAxiosGet = vi
      .spyOn(axios, "get")
      .mockImplementation(vi.fn().mockResolvedValue({ data: "graph TD;" }));

    await processFile(ghMMDFile, item, rootRemoteDir, "testLocalDir");
    let expectedPath = path.join(
      "testLocalDir",
      item.repoName,
      path.relative(rootRemoteDir, ghMMDFile.path)
    );
    expectedPath = path.join(
      path.dirname(expectedPath),
      path.basename(expectedPath, path.extname(expectedPath)) + ".md"
    );
  ...
  it("Should process and save a file different from .mmd file as .md", async () => {
    ...
    vi.spyOn(fs, "writeFileSync").mockImplementationOnce(vi.fn());
    vi.spyOn(axios, "get").mockImplementation(
      vi.fn().mockResolvedValue({ data: ghMDFile })
    );
    await processFile(ghMDFile, item, rootRemoteDir, "testLocalDir");
    let expectedPath = path.join(
      "testLocalDir",
      item.repoName,
      path.relative(rootRemoteDir, ghMDFile.path)
    );
    expectedPath = path.join(
      path.dirname(expectedPath),
      path.basename(expectedPath, path.extname(expectedPath)) + ".md"
    ...
    expect(fs.writeFileSync).toHaveBeenCalledWith(expectedPath, ghMDFile);
  });
  ...
  it("Should not process if file is different from .md or .mmd", async () => {
    ...
    vi.spyOn(fs, "writeFileSync").mockImplementationOnce(vi.fn());
    vi.spyOn(axios, "get").mockImplementation(
      vi.fn().mockResolvedValue({ data: ghNotSupportedFile })
    );
